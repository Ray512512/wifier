...
public class CityPicker implements CanShow, OnWheelChangedListener {
    ...
    public void initProvinceDatas(List<ProvinceModel> provinceList) {
         if(provinceList==null)return;
            // 获取解析出来的数据
            //*/ 初始化默认选中的省、市、区
            if (provinceList != null && !provinceList.isEmpty()) {
                mCurrentProviceName = provinceList.get(0).getName();
                List<CityModel> cityList = provinceList.get(0).getList();
            ...
            for (int i = 0; i < provinceList.size(); i++) {
                ...
                List<CityModel> cityList = provinceList.get(i).getList();
